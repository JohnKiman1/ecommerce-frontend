...
export default function AdminDashboard() {
  ...
  const statCards = [
    { title: 'Total Products', value: stats.products, icon: Package, color: 'bg-primary/100' },
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {statCards.map((stat) => (
          <div key={stat.title} className="bg-card rounded-lg shadow-sm p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground/80">{stat.title}</p>
      ...
      <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-3">
            ...
            <Link
              href="/admin/products"
              className="block w-full text-center px-4 py-2 border border-gray-300 text-foreground/80 rounded-lg hover:bg-muted transition-colors"
            >
              Manage Products
            </Link>
          </div>
